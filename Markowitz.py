...
class RiskParityPortfolio:
    ...
    def calculate_weights(self):
        ...
        """
        # 1. Identify target assets (Exclude the specific asset, e.g., SPY)
        target_assets = [c for c in df.columns if c != self.exclude]
        
        # Initialize the weights dataframe with the same index and columns as the input df
        self.portfolio_weights = pd.DataFrame(0.0, index=df.index, columns=df.columns)

        """
        TODO: Complete Task 2 Below
        """
        # 2. Extract price data for target assets
        prices = df[target_assets]
        
        # 3. Calculate daily returns
        returns = prices.pct_change()
        
        # 4. Calculate Rolling Standard Deviation (Volatility)
        # We use .shift(1) to avoid Look-Ahead Bias. 
        # We must use yesterday's volatility to determine today's allocation.
        rolling_std = returns.rolling(window=self.lookback).std().shift(1)
        
        # 5. Calculate Inverse Volatility
        # Risk Parity allocates higher weights to lower volatility assets.
        # Handle division by zero or infinite values if volatility is extremely low.
        inv_vol = 1.0 / rolling_std
        inv_vol = inv_vol.replace([np.inf, -np.inf], 0.0).fillna(0.0)
        
        # 6. Normalize Weights
        # Formula: w_i = (1 / std_i) / Sum(1 / std_j)
        sum_inv_vol = inv_vol.sum(axis=1)
        # Perform division row-wise. 
        # If the sum is 0 (e.g., during the initial warm-up period), the result will be NaN (handled later).
        weights = inv_vol.div(sum_inv_vol, axis=0)
        
        # 7. Fill NaNs with 0
        # The first 'lookback' rows will be NaN because the rolling window wasn't full yet.
        weights = weights.fillna(0.0)
        
        # 8. Assign calculated weights to the main storage DataFrame
        # Since we used 'df' to derive these, the indexes are perfectly aligned.
        self.portfolio_weights[target_assets] = weights

        """
        TODO: Complete Task 2 Above
        """
        
        # Final cleanup to ensure continuity
        self.portfolio_weights.ffill(inplace=True)
        self.portfolio_weights.fillna(0, inplace=True)
    ...
    def mv_opt(self, R_n, gamma):
        ...
        with gp.Env(empty=True) as env:
            ...
            with gp.Model(env=env, name="portfolio") as model:
                ...
                """
                # 1. Decision Variables (long-only, fully invested)
                w = model.addMVar(n, lb=0.0, ub=1.0, name="w")
                
                # 2. Objective Function: Maximize (Return - 0.5 * gamma * Risk)
                # Note: The problem asks for max(w'u - (gamma/2)w'Sigma w)
                ...
                # 3. Constraints: Budget constraint (sum(w) = 1)
